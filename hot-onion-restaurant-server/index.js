...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@projectcluster.nstns.mongodb.net/${process.env.DB_NAME}?retryWrites=true&w=majority`;
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true  });
client.connect(err => {
  const restaurantCollections = client.db("food-delivery-system").collection("restaurants");
  const orderCollection = client.db("food-delivery-system").collection("orders");
  const foodCollection = client.db("food-delivery-system").collection("foodItems");
  const userCollection = client.db("food-delivery-system").collection("allUsers");
  const supplierCollection = client.db("food-delivery-system").collection("suppliers");
